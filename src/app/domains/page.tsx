...
export default function DomainsPage() {
  ...
  return (
    <>
      ...
      {/* TLD pricing table */}
      <section className="border-y border-border-soft bg-slate-50 py-16" aria-labelledby="tld-pricing">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          />
          <div className="mt-10 overflow-x-auto rounded-2xl border border-border-soft bg-white shadow-sm">
            <table className="w-full min-w-[560px] border-collapse text-sm">
              <thead>
                <tr className="border-b border-border-soft bg-slate-50 text-left">
                  <th scope="col" className="px-5 py-3.5 font-semibold text-slate-700">Extension</th>
                  <th scope="col" className="px-5 py-3.5 font-semibold text-slate-700">Register</th>
                  <th scope="col" className="px-5 py-3.5 font-semibold text-slate-700">Renew</th>
                  <th scope="col" className="px-5 py-3.5 font-semibold text-slate-700">Transfer</th>
                  <th scope="col" className="px-5 py-3.5 font-semibold text-slate-700">Min years</th>
                </tr>
              </thead>
              <tbody>
                {tlds.map((tld) => (
                  <tr key={tld.tld} className="border-b border-border-soft last:border-0 hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-3 text-slate-700">৳{tld.registerBdt.toLocaleString()}</td>
                    <td className="px-5 py-3 text-slate-700">৳{tld.renewBdt.toLocaleString()}</td>
                    <td className="px-5 py-3 text-slate-700">৳{tld.transferBdt.toLocaleString()}</td>
                    <td className="px-5 py-3 text-slate-700">{tld.minYears} yr</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="rounded-3xl border border-border-soft bg-gradient-to-br from-slate-50 to-white p-8 shadow-sm sm:p-10">
            <div className="flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <Link
                  href="/pricing"
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-border-soft px-6 py-3 text-sm font-semibold text-slate-700 hover:border-primary/40 hover:text-primary"
                >
                  View Pricing
                </Link>
              </div>
      ...
      <section className="border-t border-border-soft bg-slate-50 py-14">
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <div className="grid gap-8 md:grid-cols-2">
            <div>
              <h2 className="text-lg font-extrabold text-slate-900">Manage everything in one place</h2>
